...
def get_parser():
    ...
    parser.add_argument("--config", type=str, default=os.path.join(SCRIPT_DIR, "config", "config_rd_ftx.json"))
...
base_log_dir = os.path.join(SCRIPT_DIR, "run", "run_rd_ftx")
...
def data_gen(num_samples, grid_x, grid_t, T, D=0.01, k=0.01):
    # RD-f(x,t) case: forcing = sin(w*x) * g(t), w~U(0,8*pi).
    g_t = _sample_time_grf(num_samples, grid_t, T, length_scale=0.1)
    x = np.linspace(0.0, 1.0, grid_x, dtype=np.float64)
    w = np.random.uniform(0.0, 8.0 * np.pi, size=(num_samples, 1))
    forcing = np.sin(w * x.reshape(1, -1))[:, :, None] * g_t[:, None, :]
...
if args.save_loss:
    ...
    loss_txt_path = os.path.join(log_dir, f"RD_ftx_Model={args.model}_Seed={args.SEED}.txt")
