...
class POD_Mamba_Scratch(nn.Module):
    def __init__(self, input_dim, output_dim, hidden_dim, num_layers=1, d_state=16):
        super(POD_Mamba_Scratch, self).__init__()
        self.in_proj = nn.Linear(input_dim, hidden_dim)
    ...
    def forward(self, x):
        x = x.permute(0, 2, 1, 3) # N, grid_t, grid_x, C
        x = x.reshape(x.size(0), x.size(1), -1) # N, grid_t, grid_x * C
        x = self.in_proj(x)
        x = self.mamba_core(x)
        x = self.out_proj(x)
