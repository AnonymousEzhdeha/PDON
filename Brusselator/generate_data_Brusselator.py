...
for i in range(N):
    ...
    tracker = [storage.tracker(interrupts=interval0)]
    sol = eq.solve(state, t_range=t_range, dt=dt, tracker=tracker)
    output1 = np.array((storage.data))[:,0,:,:]   # u field
    storage_outer1.append(output1)
finish = time.time() - start
print('Total time: ',finish)  

# Ensure all arrays have the same shape by using the minimum time dimension
min_time_steps = min([arr.shape[0] for arr in storage_outer1])
outputs_all = np.array([arr[:min_time_steps, :, :] for arr in storage_outer1]) 
