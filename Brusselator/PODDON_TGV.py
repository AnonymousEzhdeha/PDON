...
def _oss_no_rollout_im(
    ...
    for t in range(seq_len):
        ...
        rhs = v + dt_t * (-omega2 * h - damping_t * v + drive_t * u_t)
        v = rhs * inv_denom
        h = h + dt_t * v
        ys[:, t, :] = h + d_skip * u_t
...
def _oss_no_rollout_imex(
    ...
    for t in range(seq_len):
        ...
        if drive.dim() == 1:
            ...
            drive_t = drive[:, t, :]
        v = v + dt_t * (-omega2 * h - damping_t * v + drive_t * u_t)
        h = h + dt_t * v
        ys[:, t, :] = h + d_skip * u_t
...
class OSS_NO(nn.Module):
    ...
    def __init__(
        ...
        self.dt_in_proj = nn.Linear(d_model, d_model) if self.use_input_dt else None
        self.drive_in_proj = nn.Linear(d_model, d_model) if self.use_input_drive_damping else None
        self.damping_in_proj = nn.Linear(d_model, d_model) if self.use_input_drive_damping else None
        self.gate_in_proj = nn.Linear(d_model, d_model) if self.use_osc_gate else None
        ...
        self.d_skip = nn.Parameter(torch.full((d_model,), float(d_skip_init), dtype=torch.float32))
    ...
    def forward(self, x):
        ...
        if self.discretization == "IM":
            y = _oss_no_rollout_im(x, omega2, damping, drive, dt, d_skip)
        else:
            y = _oss_no_rollout_imex(x, omega2, damping, drive, dt, d_skip)
