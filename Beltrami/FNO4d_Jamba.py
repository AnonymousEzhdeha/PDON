...
import numpy as np
import torch
from time_model import GRU, LSTM, Mamba_NO, MambaConfig, OSS_NO
...
class FNO_Jamba_Layer_1(nn.Module):
    def __init__(self, modes, width, model_t_type, oss_discretization="IM", oss_dt=1.0, oss_dt_min=1e-3, oss_dt_max=1.0):
        ...
        if model_t_type == "GRU":
            ...
            self.model_t = OSS_NO(
                ...
                input_dim=width,
                output_dim=width,
                discretization=oss_discretization,
                dt=oss_dt,
                dt_min=oss_dt_min,
                dt_max=oss_dt_max,
            )
...
class FNO_Jamba_1(nn.Module):
    def __init__(self, input_dim, output_dim, modes, width, num_layers, model_t_type, oss_discretization="IM", oss_dt=1.0, oss_dt_min=1e-3, oss_dt_max=1.0):
        super(FNO_Jamba_1, self).__init__()
        self.in_proj = nn.Linear(input_dim, width)
        self.blocks = nn.ModuleList([
            FNO_Jamba_Layer_1(modes, width, model_t_type, oss_discretization, oss_dt, oss_dt_min, oss_dt_max)
...
class FNO_Jamba_2(nn.Module):
    def __init__(self, input_dim, output_dim, modes, width, num_layers, model_t_type, oss_discretization="IM", oss_dt=1.0, oss_dt_min=1e-3, oss_dt_max=1.0):
        ...
        self.in_proj = nn.Linear(input_dim, width)
        self.blocks = nn.ModuleList([
            FNO_Jamba_Layer_2(modes, width, model_t_type, oss_discretization, oss_dt, oss_dt_min, oss_dt_max)
            for _ in range(num_layers)
            ])
        self.fc1 = nn.Linear(width, width)
        self.fc2 = nn.Linear(width, width)
