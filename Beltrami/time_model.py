...
class OSS_NO(nn.Module):
    def __init__(self, d_model, input_dim, output_dim, discretization="IM", dt=1.0, dt_min=1e-3, dt_max=1.0):
        ...
        dt_init = float(max(self.dt_min + 1e-6, min(float(dt), self.dt_max - 1e-6)))
        dt_ratio = (dt_init - self.dt_min) / (self.dt_max - self.dt_min)
        dt_ratio = float(max(1e-6, min(1.0 - 1e-6, dt_ratio)))
        dt_logit_init = torch.logit(torch.tensor(dt_ratio)).item()
