...
class PCA_Layer_Encode(nn.Module):
    def __init__(self, components_, mean_):
        super().__init__()
        self.components_ = components_ # grid_x, latent_dim
        self.mean_ = mean_

    def forward(self, x): # N, grid_t, latent_dim
        x = (x - self.mean_) @ self.components_ # N_, grid_t, grid_x
        return x

class PCA_Layer_Decode(nn.Module):
    def __init__(self, components_, mean_):
        super().__init__()
        self.components_ = components_ # grid_x, latent_dim
        self.mean_ = mean_
    ...
    def forward(self, x): # N, grid_t, latent_dim
        x = x @ self.components_.T + self.mean_ # N_, grid_t, grid_x
        return x

"""class PODDON_GRU(nn.Module):
    def __init__(self, input_dim, output_dim, hidden_dim, num_layers, components_, mean_):
        super(PODDON_GRU, self).__init__()
        self.in_proj = nn.Linear(input_dim, hidden_dim)
        self.gru = nn.GRU(hidden_dim, hidden_dim, num_layers, batch_first=True)
        self.out_proj = nn.Sequential(nn.Linear(hidden_dim, output_dim), PCA_Layer_Decode(components_, mean_))  # Output layer to match input dimensions

    def forward(self, x): # N, grid_x, grid_x, grid_x, grid_t, 2
        x = self.in_proj(x) # N, grid_t, hidden_dim
        gru_out, _ = self.gru(x) # N, grid_t, hidden_dim
        out = self.out_proj(gru_out) # N, grid_t, -1
        return out
"""

class PODDON_GRU(nn.Module):
    def __init__(self, input_dim, output_dim, hidden_dim, num_layers, POD_Basis, POD_Mean, In_Basis, In_Mean):
        super(PODDON_GRU, self).__init__()
        self.in_proj = nn.Sequential(PCA_Layer_Encode(In_Basis, In_Mean), nn.Linear(input_dim, hidden_dim))
        self.gru = nn.GRU(hidden_dim, hidden_dim, num_layers, batch_first=True)
        self.out_proj = nn.Sequential(nn.Linear(hidden_dim, output_dim), PCA_Layer_Decode(POD_Basis, POD_Mean))
...
class PODDON_LSTM(nn.Module):
    def __init__(self, input_dim, output_dim, hidden_dim, num_layers, POD_Basis, POD_Mean, In_Basis, In_Mean):
        super(PODDON_LSTM, self).__init__()
        self.in_proj = nn.Sequential(PCA_Layer_Encode(In_Basis, In_Mean), nn.Linear(input_dim, hidden_dim))
...
# ============================================================================
# OSS_NO: Oscillatory State-Space Temporal Model (LinOSS-style)
# ============================================================================
...
def _oss_no_rollout_im(
    ...
    dt: torch.Tensor,
    d_skip: torch.Tensor,
    couple_omega_u: torch.Tensor,
    couple_omega_v: torch.Tensor,
    couple_damping_factor: torch.Tensor,
    couple_drive_u: torch.Tensor,
    couple_drive_v: torch.Tensor,
...
class OSS_NO(nn.Module):
    ...
    def __init__(
        ...
        if bool(robust_dt_init):
            dt_ratio = float(np.clip(dt_ratio, 1e-6, 1.0 - 1e-6))
        ...
        if self.use_coupled_oscillators:
            ...
            # PSD low-rank damping coupling to preserve dissipative structure.
            ...
            self.couple_omega_u = nn.Parameter(torch.empty(0), requires_grad=False)
            self.couple_omega_v = nn.Parameter(torch.empty(0), requires_grad=False)
            self.couple_damping_factor = nn.Parameter(torch.empty(0), requires_grad=False)
            self.couple_drive_u = nn.Parameter(torch.empty(0), requires_grad=False)
            self.couple_drive_v = nn.Parameter(torch.empty(0), requires_grad=False)
    ...
    def forward(self, x):
        ...
        if self.use_causal_prefilter:
            ...
            x_conv = torch.nn.functional.pad(x_conv, (self.prefilter_kernel_size - 1, 0))
        ...
        if self.use_input_drive_damping:
            ...
            damping = torch.nn.functional.softplus(damping_logits)
        else:
            drive = self.drive
        ...
        if self.use_osc_gate:
            gate = torch.nn.functional.silu(self.gate_in_proj(x))
            y = y * gate
...
class PODDON_OSS_NO(nn.Module):
    """
    POD-augmented Oscillatory State-Space temporal model.
    
    Combines reduced-order modeling (POD/PCA) with OSS dynamics for efficient
    spatio-temporal PDE modeling. Follows the same architecture as PODDON_Mamba
    but uses OSS_NO temporal core instead of Mamba blocks.
    
    Architecture:
    Input → PCA_Layer_Encode → Linear (d_model) → OSS_NO → Linear (output_dim) → PCA_Layer_Decode → Output
    
    The OSS_NO core learns oscillatory dynamics with constrained learnable timesteps,
    supporting both IM (implicit, dissipative) and IMEX (implicit-explicit, symplectic) schemes.
    """
    ...
    def __init__(
        ...
        robust_dt_init: bool = False,
        use_input_dt: bool = False,
        use_d_skip: bool = False,
        d_skip_init: float = 1.0,
        use_input_drive_damping: bool = False,
        input_drive_scale: float = 1.0,
        input_damping_scale: float = 1.0,
        use_osc_gate: bool = False,
        use_causal_prefilter: bool = False,
        prefilter_kernel_size: int = 3,
        use_expand_project: bool = False,
        expand_factor: int = 2,
        expand_init_scale: float = 0.02,
        use_coupled_oscillators: bool = False,
        coupling_rank: int = 4,
        coupling_scale: float = 0.05,
    ):
        """
        Initialize PODDON_OSS_NO.
        
        Args:
            input_dim (int): Input feature dimension (full space after PCA decoding)
            output_dim (int): Output feature dimension (full space)
            hidden_dim (int): Latent dimension for OSS state-space (d_model)
            POD_Basis: POD basis matrix for decoding outputs (components_.T from PCA)
            POD_Mean: POD mean vector for decoding outputs
            In_Basis: POD basis matrix for encoding inputs
            In_Mean: POD mean vector for encoding inputs
            discretization (str): OSS discretization scheme ("IM" or "IMEX")
                - "IM": Implicit Midpoint, dissipative dynamics
                - "IMEX": Implicit-Explicit, symplectic/volume-preserving
            dt (float): Initial timestep value (will be learned and constrained)
            dt_min (float): Minimum allowed timestep (lower bound)
            dt_max (float): Maximum allowed timestep (upper bound)
        """
        super(PODDON_OSS_NO, self).__init__()
        self.residual_weight = float(residual_weight)
        self.num_layers = int(max(1, num_layers))

        # Input embedding: full-space PCA decode → latent projection
        self.in_proj = nn.Sequential(
            PCA_Layer_Encode(In_Basis, In_Mean),  
            nn.Linear(input_dim, hidden_dim)
        )

        self.oss_cores = nn.ModuleList(
            [
                OSS_NO(
                    d_model=hidden_dim,
        ...
        # Output embedding: latent projection → full-space PCA encode
        self.out_proj = nn.Sequential(
            nn.Linear(hidden_dim, output_dim),
            PCA_Layer_Decode(POD_Basis, POD_Mean)
        )

    def forward(self, x):
        """
        Forward pass through POD-OSS temporal model.
        
        Args:
            x (torch.Tensor): Input of shape (batch_size, seq_len, input_dim)
                            Full-space PDE state (e.g., from spatial FNO)
        
        Returns:
            y (torch.Tensor): Output of shape (batch_size, seq_len, output_dim)
                            Full-space predicted state
        
        Computation:
            1. PCA encoding: project full-space input to latent subspace
            2. Linear projection: input_dim → hidden_dim
            3. OSS rollout: apply oscillatory dynamics with IM/IMEX scheme
            4. Linear projection: hidden_dim → output_dim
            5. PCA decoding: reconstruct to full space
        """
        # Step 1: Input projection with PCA encoding
        h = self.in_proj(x)  # (batch, seq_len, hidden_dim)

        # Step 2: Stacked OSS temporal dynamics
        for oss_core in self.oss_cores:
            y_latent = oss_core(h)  # (batch, seq_len, hidden_dim)
            if self.residual_weight != 0.0:
                h = y_latent + self.residual_weight * h
            else:
                h = y_latent

        # Step 3: Output projection with PCA decoding
        y = self.out_proj(h)  # (batch, seq_len, output_dim)

        return y


class PODDON_Mamba_Scratch(nn.Module):
    """POD wrapper around a pure-PyTorch Mamba scratch temporal core."""
    ...
    def __init__(
        ...
        super(PODDON_Mamba_Scratch, self).__init__()
    ...
    def forward(self, x):
        x_latent = self.in_proj(x)
        y_latent = self.mamba_core(x_latent)
...
class PODDON_Mamba(nn.Module):
    def __init__(
        ...
        input_dim: int,
        output_dim: int,
        POD_Basis, POD_Mean, In_Basis, In_Mean,
        ssm_cfg=None,
        attn_layer_idx=None,
        attn_cfg=None,
        norm_epsilon: float = 1e-5,
        ...
        self.embedding = nn.Sequential(PCA_Layer_Encode(In_Basis, In_Mean), nn.Linear(input_dim, d_model))
        # self.out_embedding = nn.Linear(d_model, output_dim)
        self.out_embedding = nn.Sequential(nn.Linear(d_model, output_dim), PCA_Layer_Decode(POD_Basis, POD_Mean))
...
class GalerkinTransformer(nn.Module):
    def __init__(self,
                 ...
                 mlp_dim, 
                 POD_Basis, POD_Mean, In_Basis, In_Mean,
                 dropout=0., max_len=5000,):
        super().__init__()

        self.dim_hid = dim_hid

        self.input_emb = nn.Sequential(PCA_Layer_Encode(In_Basis, In_Mean), nn.Linear(dim_in, dim_hid))
        self.decoder = nn.Sequential(nn.Linear(dim_hid, dim_out), PCA_Layer_Decode(POD_Basis, POD_Mean))
...
class Transformer(nn.Transformer):
    def __init__(self, ninput, noutput, nhidden, nhead, dim_feedforward, nlayers, 
                 POD_Basis, POD_Mean, In_Basis, In_Mean,
        ...
        self.input_emb = nn.Sequential(PCA_Layer_Encode(In_Basis, In_Mean), nn.Linear(ninput, nhidden))
        self.decoder = nn.Sequential(nn.Linear(nhidden, noutput), PCA_Layer_Decode(POD_Basis, POD_Mean))
...
class GNOT(nn.Module):
    def __init__(self, dim_in, dim_out, dim_hid, depth, heads, dim_head, n_experts, 
                 POD_Basis, POD_Mean, In_Basis, In_Mean,
                 dropout=0., max_len=5000):
